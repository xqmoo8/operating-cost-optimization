...
lambda_Pb = -Parameter.G(1,1)*Pg(1,1:OPTIONS.N_t) - Parameter.G(1,2) ;
lambda_delta = zeros(1,OPTIONS.N_t);

y = size(find(delta(2:OPTIONS.N_t) - delta(1:OPTIONS.N_t-1)==1),2);
objval_upperbound= cvx_optval + y*Parameter.C_ss(1) + sum( Parameter.E(1,2)*Pb(1,1:OPTIONS.N_t) + Parameter.E(1,3)*ones(1,OPTIONS.N_t)) ;

%% build master problem
%% delta(1,Nt); y(1,Nt-1); Pb(1,Nt); E(1,Nt); mu(1); redundant_switch(1,2) total number = 4*Nt + 2
% delta and Pg_min
A_delta_Pg_bound = zeros(OPTIONS.N_t, 4*OPTIONS.N_t)
for index_x = 1:OPTIONS.N_t
    A_delta(index_x,index_x) = 1;
end
b_y = (Pg(1,1:OPTIONS.N_t).')./OPTIONS.Pg_Min(1);
...
for index_x = 2*OPTIONS.N_t:3*OPTIONS.N_t-1
    A_benders(1, index_x) = - lambda_Pb(1,index_x-2*OPTIONS.N_t+1);
end
A_benders(1, 4*OPTIONS.N_t) = -1; 
b_benders =  - cvx_optval + sum(lambda_delta*delta_g') + sum(lambda_Pb*Pb');
...
A = [A_delta; A_benders; A_E_start];
b = [b_y; b_benders; b_E_start];

% ESM equality constraint
Aeq_E = [zeros(OPTIONS.N_t-1, 2*OPTIONS.N_t-1)  -zeros(OPTIONS.N_t-1, OPTIONS.N_t) zeros(OPTIONS.N_t-1, OPTIONS.N_t) zeros(OPTIONS.N_t-1, 1)];
for index_x = 1: OPTIONS.N_t-1
...
for index_x = 1: OPTIONS.N_t
    Aeq_B(index_x, index_x) = Pg(1,index_x);
    Aeq_B(index_x, 2*OPTIONS.N_t+index_x-1) = 1;
end
beq_B = (Ppr + Parameter.alpha * OPTIONS.P_L_TIME_off(1,1:OPTIONS.N_t)).';
