function [cvx_optval ] = operating_cost_offline( )
%% global variable
...
OPTIONS.Coupled_load(1, :) = 1 * OPTIONS.P_L_TIME_off/6;
OPTIONS.Coupled_load(2, :) = 1 * OPTIONS.P_L_TIME_off/6;
...
%     end
    ...
    offline_objval(2,:) = (offline_objval -offline_objval(1))*100/offline_objval(1);
...
%                 Pg(1:2,1:4) == OPTIONS.pg_data_wo
    ...
    if operation_mode<=3
        ...
        Pg(1,1:4) = Pg(1,1:4) + OPTIONS.P_L_TIME_off(1,1:4) - OPTIONS.P_L_TIME_on(1,1:4);
        Pg(1,5:OPTIONS.N_t) = Pg(1,5:OPTIONS.N_t) + Parameter.alpha * (OPTIONS.P_L_TIME_off(1,5:OPTIONS.N_t) - OPTIONS.P_L_TIME_on(1,5:OPTIONS.N_t));
        Pg(2,5:OPTIONS.N_t) = Pg(2,5:OPTIONS.N_t) + (1-Parameter.alpha -1/6) * (OPTIONS.P_L_TIME_off(1,5:OPTIONS.N_t) - OPTIONS.P_L_TIME_on(1,5:OPTIONS.N_t));
    end
