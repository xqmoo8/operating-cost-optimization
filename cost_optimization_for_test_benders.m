function [optimal_cost, time_val ] = cost_optimization_for_test_benders( time_slot, voya_distance, accelerate_flag_input, online_offline_input, operation_mode_input )
...
OPTIONS.P_L_Scale = [0.5 0.6 0.8 0.9 0.82 0.6 0.4 0.35 0.25 0.33 0.4 0.5 0.4 0.3 0.6 0.8 0.82 0.9 0.9 0.7 0.62 0.5 0.33 0.4 0.5 0.4]; 
...
OPTIONS.Penalty_D = 3*(2.2e-3)^(1/3)*max([2*OPTIONS.G(1,1)*OPTIONS.Pg_Max(1)+OPTIONS.G(1,2) 2*OPTIONS.G(2,1)*OPTIONS.Pg_Max(2)+OPTIONS.G(2,2) ])...
                    /(OPTIONS.Ppr_Max)^(-2/3);
...
%         cvx_begin
        ...
        %                  OPTIONS.P_L_TIME_off(1,t_index) + Ppr(1,t_index) == sum( Pg(1:OPTIONS.N_g,t_index) )
                ...
                elseif operation_mode <= 7
                    for t_index = 1:OPTIONS.N_t
                        redundent_sw_s(1:2, t_index).'*OPTIONS.Coupled_load(:, t_index) +  (1-OPTIONS.alpha - 2/6) * OPTIONS.P_L_TIME_off(1, t_index) + Ppr(1, t_index) == (Pg(1, t_index)) + Pb(1, t_index) 
                        redundent_sw_s(3:4, t_index).'*OPTIONS.Coupled_load(:, t_index) + OPTIONS.alpha * OPTIONS.P_L_TIME_off(1, t_index)  == (Pg(2, t_index)) + Pb(2, t_index)
                    end
                end
...
%         cvx_begin
        ...
        %             + OPTIONS.Xi * sum(OPTIONS.E(1:2,2).'* ones(OPTIONS.N_g,1) ,2)...
            ...
            subject to
                % the range constraints of all the variables
                Pg_on(1,1) <= delta_s(1,1) * OPTIONS.Pg_Max(1) + Pc(1,1)
                Pg_on(1,1) >= delta_s(1,1) * OPTIONS.Pg_Min(1) - Pd(1,1)
                Pg_on(2,1) <= delta_s(2,1) * OPTIONS.Pg_Max(2) + Pc(2,1)
                Pg_on(2,1) >= delta_s(2,1) * OPTIONS.Pg_Min(2) - Pd(2,1)
                ...
                if OPTIONS.P_L_TIME_off(index_time) - OPTIONS.P_L_TIME_off_avg >= 0
                    ...
                    Pb_on(1,1) <= (1-OPTIONS.alpha - 2/6)*(OPTIONS.P_L_TIME_off(index_time) - OPTIONS.P_L_TIME_off_avg)
                    Pb_on(1,1) >= 0
                    Pb_on(2,1) <= OPTIONS.Pb_Max
                    Pb_on(2,1) <= OPTIONS.alpha*(OPTIONS.P_L_TIME_off(index_time) - OPTIONS.P_L_TIME_off_avg)
                    Pb_on(2,1) >= 0
...
%                 for t_index = 1
                ...
                if operation_mode <= 3
                    for t_index = 1
                         OPTIONS.P_L_TIME_off(1,index_time) + Ppr_on(1,1) == sum( Pg_on(1:OPTIONS.N_g,1) ) + sum(Pb_on(1:OPTIONS.N_e,1))
        %                  OPTIONS.P_L_TIME_off(1,t_index) + Ppr(1,t_index) == sum( Pg(1:OPTIONS.N_g,t_index) )
                    end
                elseif operation_mode <= 7
                    for t_index = 1
                        redundent_sw_s(1,:)*OPTIONS.Coupled_load(:,index_time) + (1-OPTIONS.alpha - 2/6) * OPTIONS.P_L_TIME_off(1,index_time) + Ppr_on(1,1) == (Pg_on(1,1)) + Pb_on(1,1) 
                        redundent_sw_s(2,:)*OPTIONS.Coupled_load(:,index_time) + OPTIONS.alpha * OPTIONS.P_L_TIME_off(1,index_time)  == (Pg_on(2,1)) + Pb_on(2,1)
                    end
                end
                ...
                if operation_mode ==0
                    ...
                    Pb_on(1:2,1) == 0
                elseif operation_mode ==1
                    sum((Ppr_on(1)./2.2e-3).^(1/3)) >= OPTIONS.Distance
                elseif operation_mode ==2
                    Ppr_on(1,1) == OPTIONS.P_pr_avg;
                elseif operation_mode ==3
                    sum((Ppr_on(1)./2.2e-3).^(1/3)) >= OPTIONS.Distance 
                ...
                elseif operation_mode ==6
                    ...
                    sum((Ppr_on(5)./2.2e-3).^(1/3)) >= OPTIONS.Distance - 4*( OPTIONS.P_pr_avg ./2.2e-3).^(1/3)
                end
        cvx_end
        ...
        dual_Ss(3:4,index_time) = temp_dual_Ss;
