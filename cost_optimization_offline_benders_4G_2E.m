...
function [objval_upperbound, Pg, Pb, Ppr, cvx_optval ] = cost_optimization_subproblem( delta, Redundent_switch, operation_mode )  
global OPTIONS Parameter
%% subproblem
% cvx_begin
cvx_begin quiet 
...
objval_upperbound= cvx_optval + y*Parameter.C_ss(1)  ...
                    + sum(sum(Parameter.G(1,2)*Pg(1:2,1:OPTIONS.N_t) + Parameter.G(1,3)*delta(1:2,1:OPTIONS.N_t)  ...
                    + Parameter.G(2,2)*Pg(3:OPTIONS.N_g,1:OPTIONS.N_t) + Parameter.G(2,3)*delta(3:OPTIONS.N_g,1:OPTIONS.N_t),1),2);
% %% FIGURE PLOT
% figure
% hold on
