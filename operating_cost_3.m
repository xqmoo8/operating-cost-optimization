...
offline_objval = zeros(1,OPTIONS.N_t);
...
%                 sum(OPTIONS.Coupled_load(:,t_index)) +  (2/3)*
...
%             Pg(2,1) - OPTIONS.pg_data_wo(2,4) >= -Parameter.R_G
            ...
            E(1,1:OPTIONS.N_t-4) <=  OPTIONS.E_Max * ones(1,OPTIONS.N_t-4)
            E(1,1:OPTIONS.N_t-4) >= zeros(1,OPTIONS.N_t-4)
            E(2,1:OPTIONS.N_t-4) <=  OPTIONS.E_Max * ones(1,OPTIONS.N_t-4)
...
%             end
            ...
            for t_index = 1:OPTIONS.N_t-4
                ...
                ~Redundent_switch*OPTIONS.Coupled_load(:,t_index+4) + (1-Parameter.alpha - 1/6) * OPTIONS.P_L_TIME_on(1,t_index+4)  == Pg(2,t_index) + Pb(2,t_index)
            end
    cvx_end
