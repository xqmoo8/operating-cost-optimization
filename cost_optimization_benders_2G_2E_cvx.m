...
% delta(1,:) = ones(1, OPTIONS.N_t);
% delta(2,:)  = zeros(1, OPTIONS.N_t);
...
for index_sm = 1:20
    for operation_mode = 7:7
        ...
        if  objval_upperbound(index_sm) - objval_lowerbound(index_sm) < Parameter.error
%             break;
        end
    end
end
...
%     dual variable y3;
%     dual variable y4;
    minimize( sum(  sum(Parameter.G(1,1:2)* power(Pg(1:OPTIONS.N_g,1:OPTIONS.N_t),2) ,1) ...
                    + sum(Parameter.E(1,1)* power(Pb(1:OPTIONS.N_g,1:OPTIONS.N_t),2) ,1) ...
                    + sum(Parameter.D(1,1)* power(Pd(1:OPTIONS.N_g,1:OPTIONS.N_t),2) ,1)  ) )
%                     + sum(Parameter.E(1,2)* power(Pb(1:OPTIONS.N_g,1:OPTIONS.N_t),1) ,1) ...
%                     + sum(Parameter.E(1,2)* power(Pb(1,1:OPTIONS.N_t),1) ,1) ...
%                     + sum(Parameter.E(2,2)* power(Pb(2,1:OPTIONS.N_t),1) ,1) ...
%                     + sum(Parameter.G(2,1)* power(Pg(2,1:OPTIONS.N_t),2) ,1) ...
%                     + sum(Parameter.G(1,2)* power(Pg(1,1:OPTIONS.N_t),1) ,1) ...
%                     + sum(Parameter.G(2,2)* power(Pg(2,1:OPTIONS.N_t),1) ,1) ...
    subject to
...
%         Pd(1,1:OPTIONS.N_t) <= OPTIONS.Pb_Max * ones(1,OPTIONS.N_t)
%         Pd(1,1:OPTIONS.N_t) >= OPTIONS.Pb_Min * ones(1,OPTIONS.N_t)
...
%             for t_index = 1: OPTIONS.Fault_time
%                  OPTIONS.P_L_TIME_off(1,t_index) + Ppr(1,t_index) == sum( Pg(1:OPTIONS.N_g,t_index) )
% %                  OPTIONS.P_L_TIME_off(1,t_index) + Ppr(1,t_index) == sum( Pg(1:OPTIONS.N_g,t_index) ) + sum(Pb(1:OPTIONS.N_e,t_index))
%             end
%             for t_index = OPTIONS.Fault_time+1:OPTIONS.N_t
            for t_index = 1:OPTIONS.N_t
%                 Redundent_switch*OPTIONS.Coupled_load(:,t_index) +  (1-Parameter.alpha - 2/6) * OPTIONS.P_L_TIME_off(1,t_index) + Ppr(1,t_index) == sum(Pg(1,t_index)) + Pb(1,t_index) + Pd(1,t_index)
%                 ~Redundent_switch*OPTIONS.Coupled_load(:,t_index) + Parameter.alpha * OPTIONS.P_L_TIME_off(1,t_index)  == sum(Pg(2,t_index)) + Pb(2,t_index) + Pd(2,t_index)
                Redundent_switch*OPTIONS.Coupled_load(:,t_index) +  (1-Parameter.alpha - 2/6) * OPTIONS.P_L_TIME_off(1,t_index) + Ppr(1,t_index) == (Pg(1,t_index)) + Pb(1,t_index) 
                ~Redundent_switch*OPTIONS.Coupled_load(:,t_index) + Parameter.alpha * OPTIONS.P_L_TIME_off(1,t_index)  == (Pg(2,t_index)) + Pb(2,t_index)
            end
        end
%         Pd(:,:) == 0;
...
objval_upperbound= cvx_optval + y*Parameter.C_ss(1) ...
                  + sum(  sum( Parameter.G(1,2)* Pg(1,1:OPTIONS.N_t),2) ...
                  + sum( Parameter.G(2,2)* Pg(2,1:OPTIONS.N_t),2) ... 
                  + sum( Parameter.G(1,3)* delta_s(1,1:OPTIONS.N_t),2) ... 
                  + sum( Parameter.G(2,3)* delta_s(2,1:OPTIONS.N_t),2) ,1);
%                 + Parameter.E(1,2)*Pb(1:OPTIONS.N_e,1:OPTIONS.N_t) ,1),2); 
...
lambda_B(1,:) = -2*Parameter.E(1,1)*Pb(1,:) ;
lambda_B(2,:) = -2*Parameter.E(1,1)*Pb(2,:) ;
% lambda_B(1,:) = -2*Parameter.G(1,1)*Pg(1,:) - Parameter.G(1,2);
% lambda_B(2,:) = -2*Parameter.G(2,1)*Pg(2,:) - Parameter.G(2,2);
lambda_Pg(1,1:OPTIONS.N_t) = - 2*Parameter.G(1,1)*Pg(1,:) - lambda_B(1,:);
lambda_Pg(2,1:OPTIONS.N_t) = - 2*Parameter.G(1,1)*Pg(2,:) - lambda_B(2,:);
...
lambda_delta(1,:) = -2*Parameter.G(1,1)*Pg(1,:).^2 ;
lambda_delta(2,:) = -2*Parameter.G(2,1)*Pg(2,:).^2 ;
% lambda_delta(1,:) = -2*Parameter.G(1,1)*Pg(1,:).^2 - Parameter.G(1,2)*Pg(1,:);
% lambda_delta(2,:) = -2*Parameter.G(2,1)*Pg(2,:).^2 - Parameter.G(2,2)*Pg(2,:);
...
cvx_begin
    ...
    minimize(  sum( sum( Parameter.C_ss*startup_g(1,1:OPTIONS.N_t-1),2)  ...
                  + sum( Parameter.C_ss*startup_g(2,1:OPTIONS.N_t-1),2)  ...
                  + sum( Parameter.G(1,2)* Pg_m(1,1:OPTIONS.N_t),2) ...
                  + sum( Parameter.G(2,2)* Pg_m(2,1:OPTIONS.N_t),2) ... 
                  + sum( Parameter.G(1,3)* delta_g(1,1:OPTIONS.N_t),2) ... 
                  + sum( Parameter.G(2,3)* delta_g(2,1:OPTIONS.N_t),2) ,1) + benders_cut )
%                   + sum( Parameter.E(1,2)*Pb_m(1:2,1:OPTIONS.N_t),2) ,1) ... 
    subject to
        % delta and Pg_min
...
%         Pg(2,1:OPTIONS.N_t) <= delta_s(2,1:OPTIONS.N_t) * OPTIONS.Pg_Max(1)
        ...
        Pg_m(1,2:OPTIONS.N_t) - Pg_m(1,1:OPTIONS.N_t-1) <= Parameter.R_G
        Pg_m(1,2:OPTIONS.N_t) - Pg_m(1,1:OPTIONS.N_t-1) >= -Parameter.R_G
        Pg_m(2,2:OPTIONS.N_t) - Pg_m(2,1:OPTIONS.N_t-1) <= Parameter.R_G
        Pg_m(2,2:OPTIONS.N_t) - Pg_m(2,1:OPTIONS.N_t-1) >= -Parameter.R_G
...
%             end
            for index_t = 1:OPTIONS.N_t
                Redundent_switch_m(1,1:2)*OPTIONS.Coupled_load(:,index_t) +  (1-Parameter.alpha - 2/6) * OPTIONS.P_L_TIME_off(1,index_t) + Ppr(1,index_t) == (Pg_m(1,index_t)) + Pb(1,index_t)
                Redundent_switch_m(1,3:4)*OPTIONS.Coupled_load(:,index_t) + Parameter.alpha * OPTIONS.P_L_TIME_off(1,index_t)  == (Pg_m(2,index_t)) + Pb(2,index_t)
%                 Redundent_switch_m(1,1:2)*OPTIONS.Coupled_load(:,index_t) +  (1-Parameter.alpha - 2/6) * OPTIONS.P_L_TIME_off(1,index_t) + Ppr(1,index_t) == delta_g(1,index_t)*(Pg_m(1,index_t)) + Pb(1,index_t)
%                 Redundent_switch_m(1,3:4)*OPTIONS.Coupled_load(:,index_t) + Parameter.alpha * OPTIONS.P_L_TIME_off(1,index_t)  == delta_g(2,index_t)*(Pg_m(2,index_t)) + Pb(2,index_t)
            end
        end
        delta_g(2,1:4) == 1
